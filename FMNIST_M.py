...
def FMNIST_LIF_Q_train(X_train, y_train, X_test, y_test):
    ...
    for epoch in range(epoch_num):
        ...
        torch.save(ly1.wt_q, "./parameters_record/FMNIST/FMNIST_LIF_Q_wt1")
        torch.save(ly2.wt_q, "./parameters_record/FMNIST/FMNIST_LIF_Q_wt2")
        print("Accuracy on train data: ")
        FMNIST_LIF_Q_test(X_train, y_train, ly1.wt_q, ly2.wt_q, True)
        print("Accuracy on test data: ")
        FMNIST_LIF_Q_test(X_test, y_test, ly1.wt_q, ly2.wt_q, True)
    pass
...
def FMNIST_LIF_Q_test(X, y, wt1, wt2, not_SA):
    gpu_num = torch.cuda.device_count()
    if gpu_num == 0:
    ...
    pass
    print("conMx: " + str(conMx))
    torch.save(conMx, "./parameters_record/FMNIST/FMNIST_M_conMx")
    if not_SA:
...
def Weight_cut(inv, u_bound, l_bound):
    wt1 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt1")
    wt2 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt2")
    ...
    torch.save(wt1_cut, "./parameters_record/FMNIST/FMNIST_LIF_Q_wt1_cut")
    torch.save(wt2_cut, "./parameters_record/FMNIST/FMNIST_LIF_Q_wt2_cut")


def FMNIST_Simulated_Annealing_Optimization(
        wt1_raw, wt2_raw, X_train, y_train, wt_max, wt_min, step):  # 模拟退火优化链接权值
    alpha = 0.99  # 衰减率
    T0, Tf = 97.0, 3.0  # 起始与结束温度

    T = T0  # 当下温度
    cor_raw = FMNIST_LIF_Q_test(X_train, y_train, wt1_raw, wt2_raw, not_SA=False)
    ...
    while T > Tf:
        ...
        cor_new = FMNIST_LIF_Q_test(X_train, y_train, wt1_new, wt2_new, not_SA=False)
        ...
        print(" ")
    pass
    print("最终训练集正确率：" + str(FMNIST_LIF_Q_test(X_train, y_train, wt1_cur, wt2_cur, not_SA=False)))
    torch.save(wt1_cur, "./parameters_record/FMNIST/FMNIST_LIF_Q_wt1_SA")
    torch.save(wt2_cur, "./parameters_record/FMNIST/FMNIST_LIF_Q_wt2_SA")
...
def FMNIST_LIF_Q():  # -->
    ...
    FMNIST_LIF_Q_train(X_train, y_train, X_test, y_test)
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))

    wt1 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt1_cut")
    wt2 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt2_cut")
    FMNIST_Simulated_Annealing_Optimization(wt1, wt2, X_train, y_train, 1.8, -0.5, 0.1)'''

    # Test
    wt1 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt1", map_location=torch.device('cpu'))
    wt2 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt2", map_location=torch.device('cpu'))
    #print("Train set: ")
    #FMNIST_LIF_Q_test(X_train, y_train, wt1, wt2, not_SA=True)
    print("Test set: ")
    FMNIST_LIF_Q_test(X_test, y_test, wt1, wt2, not_SA=False)


def FMNIST_LIF_Q_memristor():  # 最终训练好的权值放到忆阻器硬件上后的结果
    # 将电阻分为24个档位
    tap_num = 24
    ...
    wt1 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt1_SA")
    wt2 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_Q_wt2_SA")
