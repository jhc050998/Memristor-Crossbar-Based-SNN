...
def JHUI_LIF_Q_train(X_train, y_train, X_test, y_test):
    # Network layout
    ly1 = SNN.LIF_QLayer(inF=34, outF=60, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5)
    ly2 = SNN.LIF_QLayer(inF=60, outF=2, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5)
    ...
    for epoch in range(epoch_num):
        ...
        pass
        '''# 每轮测试一次
        acc = JHUI_LIF_Q_test(X_test, y_test, ly1.wt, ly2.wt, False)
        total_acc.append(acc)'''
    pass
    torch.save(ly1.wt_q, "./parameters_record/JHUI/JHUI_LIF_Q_wt1")
    torch.save(ly2.wt_q, "./parameters_record/JHUI/JHUI_LIF_Q_wt2")
    torch.save(total_loss, "./parameters_record/JHUI/JHUI_Q_loss")
    # torch.save(total_acc, "./parameters_record/JHUI/JHUI_Q_acc")
...
def JHUI_LIF_Q_test(X, y, wt1, wt2, not_SA):
    # Network layout
    ly1 = SNN.LIF_QLayer(inF=34, outF=60, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=wt1)
    ly2 = SNN.LIF_QLayer(inF=60, outF=2, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=wt2)
...
def Weight_cut(inv, u_bound, l_bound):
    wt1 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt1")
    wt2 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt2")
    ...
    torch.save(wt1_cut, "./parameters_record/JHUI/JHUI_LIF_Q_wt1_cut")
    torch.save(wt2_cut, "./parameters_record/JHUI/JHUI_LIF_Q_wt2_cut")


def JHUI_Simulated_Annealing_Optimization(wt1_raw, wt2_raw, X_train, y_train, wt_max, wt_min, step):  # 模拟退火优化链接权值
    alpha = 0.99  # 衰减率
    T0, Tf = 97.0, 3.0  # 起始与结束温度

    T = T0  # 当下温度
    cor_raw = JHUI_LIF_Q_test(X_train, y_train, wt1_raw, wt2_raw, not_SA=False)
    ...
    while T > Tf:
        ...
        changeP1 = torch.where(changeP1 > 1.0 - 1e-3, 1.0, 0.0)
        changeM1 = torch.rand_like(wt1_raw)
        changeM1 = torch.where(changeM1 < 1e-3, -1.0, 0.0)

        changeP2 = torch.rand_like(wt2_raw)
        changeP2 = torch.where(changeP2 > 1.0 - 1e-3, 1.0, 0.0)
        changeM2 = torch.rand_like(wt2_raw)
        changeM2 = torch.where(changeM2 < 1e-3, -1.0, 0.0)
        ...
        cor_new = JHUI_LIF_Q_test(X_train, y_train, wt1_new, wt2_new, not_SA=False)
        ...
        print(" ")
    pass
    print("最终训练集正确率：" + str(JHUI_LIF_Q_test(X_train, y_train, wt1_cur, wt2_cur, not_SA=False)))
    torch.save(wt1_cur, "./parameters_record/JHUI/JHUI_LIF_Q_wt1_SA")
    torch.save(wt2_cur, "./parameters_record/JHUI/JHUI_LIF_Q_wt2_SA")


def JHUI_LIF_Q():  # 95.43/92.61 --> 96.57/93.75
    # Data prepare
    X, y = Ds.JHUI_loader(isSave=False, isLoad=True)

    # Read out data
    f = 0
    siz = 176
    X_train, y_train = torch.cat(
        (X[0:f * siz], X[f * siz + siz:351]), dim=0), torch.cat((y[0:f * siz], y[f * siz + siz:351]), dim=0)
    X_test, y_test = X[f * siz:f * siz + siz], y[f * siz:f * siz + siz]

    # Train
    '''time_start = time.time()
    JHUI_LIF_Q_train(X_train, y_train, X_test, y_test)
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))

    wt1 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt1_cut")
    wt2 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt2_cut")
    JHUI_Simulated_Annealing_Optimization(wt1, wt2, X_train, y_train, 1.6, -0.7, 0.1)

    # Test
    wt1 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt1")  #_SA
    wt2 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt2")
    print("Train set: ")
    JHUI_LIF_Q_test(X_train, y_train, wt1, wt2, not_SA=True)
    print("Test set: ")
    JHUI_LIF_Q_test(X_test, y_test, wt1, wt2, not_SA=True)'''
    ...
    for i in range(ep):
        ...
        T4[0, i], T4[1, i], T4[2, i], T4[3, i] = JHUI_LIF_Q_test(X_test, y_test, wt1, wt2, False)
        print("第" + str(i + 1) + "轮结束。")
    pass
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))
    # print(T4)
    torch.save(T4, "./parameters_record/JHUI/JHUI_Q_T4")


def JHUI_LIF_Q_memristor():  # 最终训练好的权值放到忆阻器硬件上后的结果
    # 将电阻分为24个档位
    tap_num = 24
    ...
    wt1 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt1_SA")
    wt2 = torch.load("./parameters_record/JHUI/JHUI_LIF_Q_wt2_SA")
    ...
    # 权值的24个档位
    weight_24 = numpy.arange(-7, 17, 1)
    print(weight_24)
    weight_24 = weight_24 / 10.0  # 权值应是的值
    print(weight_24)
    weight_24 = (weight_24 + 0.8) / 250.0  # 调整使权值落在忆阻器各态范围内
    print(weight_24)
    ...
    # print(cond_res)
    wt1_m = wt1 * 10.0 + 7.0
    wt2_m = wt2 * 10.0 + 7.0  # 对应0-23，共24个档位
    for i in range(tap_num):
        # print("档位" + str(i) + "记录完成。")
    ...
    ly1 = SNN.LIF_QLayer(inF=34, outF=60, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=None, wt_q=wt1_m)
    ly2 = SNN.LIF_QLayer(inF=60, outF=2, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=None, wt_q=wt2_m)
    ...
    for bi in range(bn):
        ...
        # Forward propagation
        t0 = 1.0 - data
        t1 = ly1.forward_Memristor(bs, t0, factor_V=-8.0, R0=2500.0)
        t2 = ly2.forward_Memristor(bs, t1, factor_V=-8.0, R0=2500.0)
...
def main():
    JHUI_LIF_Q()
    # Weight_cut(inv=0.1, u_bound=1.6, l_bound=-0.7)
    # JHUI_LIF_Q_memristor()

    T4 = torch.load("./parameters_record/JHUI/JHUI_Q_T4")
    print(T4.size())
    print(T4)
    ...
    TP, TN, FP, FN = T4_g[0], T4_g[1], T4_g[2], T4_g[3]
