...
def JHUI_LIF_train(X_train, y_train, X_test, y_test):
    # Network layout
    ly1 = SNN.LIFLayer(inF=34, outF=60, T=4.0, dt=0.01)
    ly2 = SNN.LIFLayer(inF=60, outF=2, T=4.0, dt=0.01)
    ...
    for epoch in range(epoch_num):
        ...
        pass
        '''#每轮测试一次
        torch.save(ly1.wt, "./parameters_record/JHUI/JHUI_LIF_wt1")
        torch.save(ly2.wt, "./parameters_record/JHUI/JHUI_LIF_wt2")
        acc = JHUI_LIF_test(X_test, y_test)
        total_acc.append(acc)'''
    pass
    torch.save(ly1.wt, "./parameters_record/JHUI/JHUI_LIF_wt1")
    torch.save(ly2.wt, "./parameters_record/JHUI/JHUI_LIF_wt2")
    torch.save(total_loss, "./parameters_record/JHUI/JHUI_loss")
    # torch.save(total_acc, "./parameters_record/JHUI/JHUI_acc")


def JHUI_LIF_test(X, y):
    # Network layout
    wt1 = torch.load("./parameters_record/JHUI/JHUI_LIF_wt1")
    wt2 = torch.load("./parameters_record/JHUI/JHUI_LIF_wt2")

    ly1 = SNN.LIFLayer(inF=34, outF=60, T=4.0, dt=0.01, wt=wt1)
    ly2 = SNN.LIFLayer(inF=60, outF=2, T=4.0, dt=0.01, wt=wt2)
...
def JHUI_LIF():  # 97.14/94.32
    # Data prepare
    X, y = Ds.JHUI_loader(isSave=True, isLoad=False)
    ...
    JHUI_LIF_train(X_train, y_train, X_test, y_test)
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))

    # Test
    print("Train set: ")
    JHUI_LIF_test(X_train, y_train)
    print("Test set: ")
    JHUI_LIF_test(X_test, y_test)'''
    ...
    for i in range(ep):
        JHUI_LIF_train(X_train, y_train, X_test, y_test)
        T4[0, i], T4[1, i], T4[2, i], T4[3, i] = JHUI_LIF_test(X_test, y_test)
        print("第" + str(i + 1) + "轮结束。")
    pass
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))
    print(T4)
    torch.save(T4, "./parameters_record/JHUI/JHUI_T4")


def main():
    # JHUI_LIF()

    T4 = torch.load("./parameters_record/JHUI/JHUI_T4")
