...
def BUPA_LIF_Q_train(X_train, y_train, X_test, y_test):
    # Network layout
    ly1 = SNN.LIF_QLayer(inF=5, outF=10, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5)
    ly2 = SNN.LIF_QLayer(inF=10, outF=2, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5)
    ...
    for epoch in range(epoch_num):
        ...
        pass
        '''# 每轮测试一次
        acc = BUPA_LIF_Q_test(X_test, y_test, ly1.wt, ly2.wt, False)
        total_acc.append(acc)'''
    pass
    torch.save(ly1.wt_q, "./parameters_record/BUPA/BUPA_LIF_Q_wt1")
    torch.save(ly2.wt_q, "./parameters_record/BUPA/BUPA_LIF_Q_wt2")
    torch.save(total_loss, "./parameters_record/BUPA/BUPA_Q_loss")
    # torch.save(total_acc, "./parameters_record/BUPA/BUPA_Q_acc")
...
def BUPA_LIF_Q_test(X, y, wt1, wt2, not_SA):
    # Network layout
    ly1 = SNN.LIF_QLayer(inF=5, outF=10, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=wt1)
    ly2 = SNN.LIF_QLayer(inF=10, outF=2, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=wt2)
...
def Weight_cut(inv, u_bound, l_bound):
    wt1 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt1")
    wt2 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt2")
    ...
    torch.save(wt1_cut, "./parameters_record/BUPA/BUPA_LIF_Q_wt1_cut")
    torch.save(wt2_cut, "./parameters_record/BUPA/BUPA_LIF_Q_wt2_cut")


def BUPA_Simulated_Annealing_Optimization(wt1_raw, wt2_raw, X_train, y_train, wt_max, wt_min, step):  # 模拟退火优化链接权值
    alpha = 0.99  # 衰减率
    T0, Tf = 97.0, 3.0  # 起始与结束温度

    T = T0  # 当下温度
    cor_raw = BUPA_LIF_Q_test(X_train, y_train, wt1_raw, wt2_raw, not_SA=False)
    ...
    while T > Tf:
        ...
        changeP1 = torch.where(changeP1 > 0.4, 5.0, 0.0)
        changeM1 = torch.rand_like(wt1_raw)
        changeM1 = torch.where(changeM1 < 0.6, -5.0, 0.0)

        changeP2 = torch.rand_like(wt2_raw)
        changeP2 = torch.where(changeP2 > 0.8, 1.0, 0.0)
        changeM2 = torch.rand_like(wt2_raw)
        changeM2 = torch.where(changeM2 < 0.2, -1.0, 0.0)
        ...
        cor_new = BUPA_LIF_Q_test(X_train, y_train, wt1_new, wt2_new, not_SA=False)
        ...
        print(" ")
    pass
    print("最终训练集正确率：" + str(BUPA_LIF_Q_test(X_train, y_train, wt1_cur, wt2_cur, not_SA=False)))
    torch.save(wt1_cur, "./parameters_record/BUPA/BUPA_LIF_Q_wt1_SA")
    torch.save(wt2_cur, "./parameters_record/BUPA/BUPA_LIF_Q_wt2_SA")


def BUPA_LIF_Q():  # 76.16/72.83 --> 77.91/73.41
    # Data prepare
    X, y = Ds.BUPA_loader(isSave=False, isLoad=True)
    ...
    BUPA_LIF_Q_train(X_train, y_train, X_test, y_test)
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))

    wt1 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt1_cut")
    wt2 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt2_cut")
    BUPA_Simulated_Annealing_Optimization(wt1, wt2, X_train, y_train, 2.2, -0.1, 0.1)

    # Test
    wt1 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt1")  #_SA
    wt2 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt2")
    print("Train set: ")
    BUPA_LIF_Q_test(X_train, y_train, wt1, wt2, not_SA=True)
    print("Test set: ")
    BUPA_LIF_Q_test(X_test, y_test, wt1, wt2, not_SA=True)'''
    ...
    for i in range(ep):
        ...
        T4[0, i], T4[1, i], T4[2, i], T4[3, i] = BUPA_LIF_Q_test(X_test, y_test, wt1, wt2, False)
        print("第" + str(i + 1) + "轮结束。")
    pass
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))
    # print(T4)
    torch.save(T4, "./parameters_record/BUPA/BUPA_Q_T4")


def BUPA_LIF_Q_memristor():  # 最终训练好的权值放到忆阻器硬件上后的结果
    # 将电阻分为24个档位
    tap_num = 24
    ...
    wt1 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt1_SA")
    wt2 = torch.load("./parameters_record/BUPA/BUPA_LIF_Q_wt2_SA")
    ...
    # 权值的24个档位
    weight_24 = numpy.arange(-1, 23, 1)
    print(weight_24)
    weight_24 = weight_24 / 10.0  # 权值应是的值
    print(weight_24)
    weight_24 = (weight_24 + 0.2) / 250.0  # 调整使权值落在忆阻器各态范围内
    print(weight_24)
    ...
    # print(cond_res)
    wt1_m = wt1 * 10.0 + 1.0
    wt2_m = wt2 * 10.0 + 1.0  # 对应0-23，共24个档位
    for i in range(tap_num):
        # print("档位" + str(i) + "记录完成。")
    ...
    ly1 = SNN.LIF_QLayer(inF=5, outF=10, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=None, wt_q=wt1_m)
    ly2 = SNN.LIF_QLayer(inF=10, outF=2, T=4.0, dt=0.01, Lb=0.0, Hb=0.40, QN=5, wt=None, wt_q=wt2_m)
    ...
    for bi in range(bn):
        ...
        # Forward propagation
        t0 = 1.0 - data
        t1 = ly1.forward_Memristor(bs, t0, factor_V=-2.0, R0=2500.0)
        t2 = ly2.forward_Memristor(bs, t1, factor_V=-2.0, R0=2500.0)
...
def main():
    # BUPA_LIF_Q()
    # Weight_cut(inv=0.1, u_bound=2.2, l_bound=-0.1)
    # BUPA_LIF_Q_memristor()

    T4 = torch.load("./parameters_record/BUPA/BUPA_Q_T4")
    print(T4.size())
    print(T4)
