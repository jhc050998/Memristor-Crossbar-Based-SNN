...
def WDBC_LIF_train(X_train, y_train, X_test, y_test):
    # Network layout
    ly1 = SNN.LIFLayer(inF=30, outF=40, T=4.0, dt=0.01)
    ly2 = SNN.LIFLayer(inF=40, outF=2, T=4.0, dt=0.01)
    ...
    for epoch in range(epoch_num):
        ...
        pass
        '''# 每轮测试一次
        torch.save(ly1.wt, "./parameters_record/WDBC/WDBC_LIF_wt1")
        torch.save(ly2.wt, "./parameters_record/WDBC/WDBC_LIF_wt2")
        acc = WDBC_LIF_test(X_test, y_test)
        total_acc.append(acc)'''
    pass
    torch.save(ly1.wt, "./parameters_record/WDBC/WDBC_LIF_wt1")
    torch.save(ly2.wt, "./parameters_record/WDBC/WDBC_LIF_wt2")
    torch.save(total_loss, "./parameters_record/WDBC/WDBC_loss")
    # torch.save(total_acc, "./parameters_record/WDBC/WDBC_acc")


def WDBC_LIF_test(X, y):
    # Network layout
    wt1 = torch.load("./parameters_record/WDBC/WDBC_LIF_wt1")
    wt2 = torch.load("./parameters_record/WDBC/WDBC_LIF_wt2")

    ly1 = SNN.LIFLayer(inF=30, outF=40, T=4.0, dt=0.01, wt=wt1)
    ly2 = SNN.LIFLayer(inF=40, outF=2, T=4.0, dt=0.01, wt=wt2)

    # Testing Process
    correct = 0
    TP, TN, FP, FN = 0, 0, 0, 0
    # TPR_y, FPR_x = torch.zeros(X.size()[0]), torch.zeros(X.size()[0])

    sn, bs = X.size()[0], 1  # number of data samples, batch size
    bn = int(math.ceil(sn / bs))  # number of batches
    for bi in range(bn):
        if (bi + 1) * bs > sn:
    ...
    print("Acc: " + str(int(correct)) + "/" + str(sn), end="")
    print("(%.3f %%)" % (100. * correct / sn))
    print(" ")

    print("TPR: " + str(TPR_y))
    print("FPR: " + str(FPR_x))
    torch.save(TPR_y, "./parameters_record/WDBC/WDBC_TPR")
    torch.save(FPR_x, "./parameters_record/WDBC/WDBC_FPR")'''

    return TP, TN, FP, FN  # correct/sn


def WDBC_LIF():  # 97.89/97.90
    # Data prepare
    ...
    WDBC_LIF_train(X_train, y_train, X_test, y_test)
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))

    # Test
    print("Train set: ")
    WDBC_LIF_test(X_train, y_train)
    print("Test set: ")
    WDBC_LIF_test(X_test, y_test)'''
    ...
    for i in range(ep):
        WDBC_LIF_train(X_train, y_train, X_test, y_test)
        T4[0, i], T4[1, i], T4[2, i], T4[3, i] = WDBC_LIF_test(X_test, y_test)
        print("第" + str(i + 1) + "轮结束。")
    pass
    time_end = time.time()
    print("Time consuming: %.3f s" % (time_end - time_start))
    print(T4)
    torch.save(T4, "./parameters_record/WDBC/WDBC_T4")


def main():
    # WDBC_LIF()
    T4 = torch.load("./parameters_record/WDBC/WDBC_T4")
    # print(T4.size())
