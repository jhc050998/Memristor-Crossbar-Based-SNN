...
def FMNIST_LIF_train(X_train, y_train, X_test, y_test):
    ...
    for epoch in range(epoch_num):
        ...
        torch.save(ly1.wt, "./parameters_record/FMNIST/FMNIST_LIF_wt1")
        torch.save(ly2.wt, "./parameters_record/FMNIST/FMNIST_LIF_wt2")
        print("Accuracy on train data: ")
        FMNIST_LIF_test(X_train, y_train)
        print("Accuracy on test data: ")
        FMNIST_LIF_test(X_test, y_test)
    pass


def FMNIST_LIF_test(X, y):
    gpu_num = torch.cuda.device_count()
    if gpu_num == 0:
    ...
    # Network layout
    wt1 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_wt1")
    wt2 = torch.load("./parameters_record/FMNIST/FMNIST_LIF_wt2")
...
def FMNIST_LIF():  #
    ...
    FMNIST_LIF_train(X_train, y_train, X_test, y_test)
    time_end = time.time()
